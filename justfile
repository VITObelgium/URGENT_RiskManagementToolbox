...
[group('install')]
[doc('Install all dependencies for development (Ubuntu only)')]
install-dev: install-python-and-uv install-latex install-docker verify-docker install-xterm
    printf "\n==== Installing development packages and pre-commit hooks... ====\n\n"
    uv sync --all-groups
    uv run pre-commit install
    printf "\n\033[0;32m==== Development setup complete. ====\033[0m\n\n"
